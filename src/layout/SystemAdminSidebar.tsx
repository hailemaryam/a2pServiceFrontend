...
const SystemAdminSidebar: React.FC = () => {
  ...
  // Set submenu height when opened
  useEffect(() => {
    if (openSubmenu !== null) {
  ...
  return (
    <aside
      className={`fixed mt-16 flex flex-col lg:mt-0 top-0 px-4 sm:px-5 left-0 bg-white dark:bg-gray-900 dark:border-gray-800 text-gray-900 h-[calc(100vh-4rem)] lg:h-screen transition-[width,transform] duration-300 ease-in-out z-50 border-r border-gray-200 
    ...
    >
      {/* Logo */}
      <div
        className={`py-6 sm:py-8 flex ${!isExpanded && !isHovered ? "lg:justify-center" : "justify-start"
          }`}
      >
        <Link to="/admin">
          {isExpanded || isHovered || isMobileOpen ? (
