    ...
    <div className="fixed inset-0 z-99999 flex items-center justify-center bg-black/50 p-4">
      <div className="relative w-full max-w-lg rounded-2xl border border-gray-200 bg-white p-6 shadow-theme-xl dark:border-gray-800 dark:bg-gray-900 max-h-[90vh] overflow-y-auto">
        ...
        {isLoading ? (
          ...
          <div className="py-10 text-center text-error-500">Failed to load transaction details.</div>
        ) : (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <p className="text-sm text-gray-500 dark:text-gray-400">SMS Credits</p>
            ...
            <div className="pt-4 border-t border-gray-100 dark:border-gray-800">
               ...
               <Badge 
                 color={
                   transaction.paymentStatus === "SUCCESSFUL" ? "success" : 
                   transaction.paymentStatus === "FAILED" ? "error" : 
                   transaction.paymentStatus === "IN_PROGRESS" ? "warning" : "default"
                 }
               >
            ...
            {transaction.smsPackage && (
              <div className="pt-4 border-t border-gray-100 dark:border-gray-800">
                <p className="text-sm text-gray-500 dark:text-gray-400 mb-2">Package Details</p>
                <div className="rounded-lg bg-gray-50 p-3 dark:bg-gray-800">
                  <p className="text-sm font-medium text-gray-900 dark:text-white">
                    {transaction.smsPackage.description || "Custom Package"}
                  </p>
                  <p className="text-xs text-gray-500 dark:text-gray-400">
                    Range: {transaction.smsPackage.minSmsCount} - {transaction.smsPackage.maxSmsCount || "Unlimited"} SMS
                  </p>
                </div>
              </div>
            )}
      ...
      <div className="min-h-screen rounded-2xl border border-gray-200 bg-white px-5 py-7 dark:border-gray-800 dark:bg-white/[0.03] xl:px-10 xl:py-12">
        ...
        <div className="overflow-x-auto">
          {isLoading ? (
            ...
            <div className="py-10 text-center text-error-500">Failed to load transactions.</div>
                ...
                {transactions.map((tx) => (
                  ...
                  >
                    <td className="px-4 py-4 text-sm text-gray-700 dark:text-gray-300 whitespace-nowrap">
                      {new Date(tx.createdAt).toLocaleDateString()} <span className="text-xs text-gray-500 ml-1">{new Date(tx.createdAt).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                    </td>
                    ...
                    <td className="px-4 py-4 text-center">
                      <Badge 
                        color={
                           tx.paymentStatus === "SUCCESSFUL" ? "success" : 
                           tx.paymentStatus === "FAILED" ? "error" : 
                           tx.paymentStatus === "IN_PROGRESS" ? "warning" : "default"
                        }
                      >
                        {tx.paymentStatus}
                      </Badge>
